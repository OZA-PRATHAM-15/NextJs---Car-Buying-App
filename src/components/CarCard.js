...
const CarCard = ({
  ...
  const sendHoverAnalytics = async (carId) => {
    try {
      await axiosInstance.post("/analytics/hover", { carId });
    } catch (error) {
  ...
  const logVisit = async () => {
    try {
      ...
      await axiosInstance.post("/cars/visit", {
        carId: car._id,
        userId: userId || null,
      });
    } catch (error) {
  ...
  const getFeatureIcon = (feature) => {
    ...
    if (f.includes("leather")) return <FaCouch style={featureIconStyle} />;
    if (f.includes("sunroof")) return <FaSun style={featureIconStyle} />;
    if (f.includes("bluetooth"))
      return <FaBluetooth style={featureIconStyle} />;
    if (f.includes("cruise")) return <FaCarSide style={featureIconStyle} />;
    if (f.includes("monitoring")) return <FaWrench style={featureIconStyle} />;
  ...
  return (
    ...
    >
      <div style={{ position: "relative", overflow: "hidden" }}>
        ...
        />
        {showFuelTag && (
          <span style={fuelTagStyle}>
            {car.specifications?.fuel_type || "N/A"}
          </span>
        )}
      </div>
      ...
      <div style={contentStyle}>
        ...
        <Link href={`/cars/${car._id}`} legacyBehavior>
          <a
            target="_blank"
