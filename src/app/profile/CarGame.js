...
const CarGame = () => {
    ...
    useEffect(() => {
        ...
        const lanes = [80, 220]; // Two lanes
        let car = { x: lanes[0], y: canvas.height - 120, width: 40, height: 80 };
        let currentLane = 0; // Player's starting lane
        let obstacles = [];
            ...
            for (let i = 0; i < obstacles.length; i++) {
                ...
                if (obstacle.y > canvas.height) {
                    ...
                    setScore((prevScore) => {
                        ...
                        if (newScore % 10 === 0) {
                            setObstacleSpeed((prevSpeed) => prevSpeed + 1); // Increase speed
                            setObstacleInterval((prevInterval) => Math.max(prevInterval - 200, 500)); // Decrease interval
                        }
        ...
        const startGame = () => {
            setGameOver(false);
            setScore(0); // Reset score only at the start of the game
            setObstacleSpeed(5);
    ...
    return (
        <div className={styles.gameContainer}>
            ...
            {gameStarted ? (
                <>
                    <p>Score: {score}</p>
                    <canvas ref={canvasRef} width="300" height="500" className={styles.canvas}></canvas>
                    {gameOver && (
